-- Sprint 6 Day 11: Review feedback SQL slices (v1)
--
-- These queries summarize reviewed case outcomes for threshold/exclusion tuning.
-- Replace selector values as needed before execution.
...
FROM reviewed
GROUP BY risk_band
ORDER BY CASE risk_band WHEN 'high' THEN 0 WHEN 'medium' THEN 1 WHEN 'low' THEN 2 ELSE 99 END,
         risk_band;

WITH reviewed AS (
  SELECT cr.score_id, cr.disposition
  FROM case_reviews cr
  JOIN fraud_scores fs ON fs.id = cr.score_id
  WHERE cr.feature_version = 'feature_v1'
    AND cr.ruleset_version = 'scoring_rules_v1'
...
ORDER BY reason_code ASC,
         CASE risk_band WHEN 'high' THEN 0 WHEN 'medium' THEN 1 WHEN 'low' THEN 2 ELSE 99 END,
         risk_band ASC;
