...
, reason_rows AS (
  SELECT r.score_id, r.disposition, ff.reason_code
  FROM reviewed r
  JOIN fraud_flags ff ON ff.score_id = r.score_id
)
SELECT
  reason_code,
  COUNT(DISTINCT score_id) AS reviewed_case_count,
  COUNT(DISTINCT CASE WHEN disposition = 'false_positive' THEN score_id END) AS false_positive_count,
  CASE WHEN COUNT(DISTINCT score_id) = 0 THEN 0
       ELSE ROUND(
         1.0 * COUNT(DISTINCT CASE WHEN disposition = 'false_positive' THEN score_id END)
...
, reason_rows AS (
  SELECT r.score_id, r.disposition, r.risk_band, ff.reason_code
  FROM reviewed r
  JOIN fraud_flags ff ON ff.score_id = r.score_id
)
SELECT
